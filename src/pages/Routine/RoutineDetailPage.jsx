...
const RoutineDetailPage = ({ routineId }) => {
    ...
    return (
      <div className="routine-detail-page">
        <RoutineDetailHeader
          ...
          showBackButton={true}
        />
        <p className="routine-detail-page__error-message">{detailError}</p>
      </div>
    );
  }
  ...
  // 수정 페이지로 이동
  const handleEdit = () => {
    navigate(`/?panel=routine-form&routineId=${routineId}`);
  };
  ...
  const handleDelete = async () => {
    if (window.confirm(`${routineDetail.name} 루틴을 정말 삭제하시겠습니까?`)) {
      try {
        await deleteRoutineHandler(routineId);
  ...
  const handleStartRoutine = () => {
    alert(`루틴 시작: ${routineDetail.name}!`);
    // 루틴 시작 로직 (타이머 시작, 운동 기록 등)
  };

  const handleEndRoutine = () => {
    alert(`루틴 종료: ${routineDetail.name}!`);
    // 루틴 종료 로직 (결과 저장 등)
  };
  ...
  const currentExercises = routineDetail.exercises || [];
  ...
  return (
    <div className="routine-detail-page">
      <RoutineDetailHeader
        title={
          routineDetail.routineType === "Challenge"
            ? "챌린지 상세"
            : "루틴 상세"
        }
        onEdit={handleEdit}
